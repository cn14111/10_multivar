...
```
two facets and one of the them showing slope and error
# mean_dayl.s_Apr (mean day length in April)
the first model shows the slope of that relationship is greater than 1 <- positive relationship (non multicollinear model)
the second model is strength explained by precipitation (lm_prp.oct)
effect of mean day length in april is negative in the fourth model (lm_2var.cor) (multicollinear model)

#sum_prcp.mm_Oct
Two uncorrelated variables-- still have very similar estimates of slopes
...
direction and magnitude of the estimates change with multicollinearity model
...
destfile = "cool_animation.gif", mode = "wb")

knitr::include_graphics("cool_animation.gif")
```
...
PC1 has high positive rates and negative rates with variables related to daylength

```{r PC2 weights}
mod_pca$rotation %>%
  as.data.frame() %>%
  rownames_to_column(var = "var") %>%
  ggplot(aes(x = reorder(var,desc(PC2)), 
             y = PC2))+
  geom_bar(stat = "identity", 
           aes(fill = PC2), 
           show.legend = F)+
